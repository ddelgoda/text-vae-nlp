...
- **The knee is not a recommendation.** With every run collapsed, the reconstruction axis measures the error of a near-constant decoder and the KL axis measures a sum of tiny per-dimension residuals that grows with dimension count. Both axes therefore track `latent_dim` rather than any reconstruction–compression trade-off, the front reduces to "smaller is better", and the knee falls at the smallest value in the sweep. Extending the grid below `ld=4` would likely move it. `artifacts/sweet_spot.json` is retained as a record of what the selection procedure returns, not as a preferred configuration.
...
- **Active-units check** (Burda, Grosse & Salakhutdinov, 2015 — `A_i = Var_x(E_q[z_i|x])`, active if `A_i > 0.01`): no active dimensions in any of the 75 frozen runs, at every latent_dim (4–64) and every β (0.1–4.0).
  - Measured in-domain on held-out AG News, the split these models validated on. Per-dimension variance ranges 1.04e-4 to 4.52e-4 (mean 1.85e-4) — roughly 20–100× below the active threshold.
  - Threshold sweep: zero active units at every threshold down to 0.0005, a 20× looser bar than the standard cutoff.
  - 0.01 is the field-standard AU threshold, and is dimensionless by construction — a fraction of the prior's own variance (`Var[z] = 1` under the `N(0,I)` prior). This model's KL term is the same closed-form KL to `N(0,I)` used in the original AU literature, so the cutoff transfers without rescaling. It is a convention from the IWAE literature rather than a derived quantity; the result here does not depend on its exact value.
  - An earlier version of this check used STS-B, which the Phase 1 models never saw. Those results agreed on the conclusion but showed 6–7× larger variance (7.8e-4 to 3.0e-3), consistent with out-of-distribution input perturbing the encoder rather than with latent signal. Both result sets are retained in `artifacts/`.
  - No bimodality in either set: a single tight cluster, not a split between "real" units and a noise floor — the signature of uniform, not partial, collapse.
  - Wording note: an earlier version reported "0 of 1860 active units". 1860 is the sum of latent dimensions across 75 separate models, not one latent space.
- **Scope**: 1,000 training examples, 500 validation, 5 epochs. Under this architecture and training regime, every tested frozen configuration collapsed. This does not establish that frozen-encoder embedding VAEs collapse in general.
- **Conclusion**: the frozen sweep shows no detectable input-dependent signal in the posterior means at any tested threshold. No configuration in the grid is a working VAE, and the ordering between them is an artefact of the sweep parameterisation rather than a ranking. The Phase 2 model was trained under a different recipe (unfrozen encoder, β warm-up, free bits); it has since been checked separately and is also collapsed — see `01_phase2_latent.md`.
