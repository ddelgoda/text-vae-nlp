...
def plot_pareto(df: pd.DataFrame, front: pd.DataFrame, sweet: pd.Series, outdir: Path) -> None:
   outdir.mkdir(parents=True, exist_ok=True)
   plt.figure()
   plt.scatter(df["kl_loss"], df["recon_loss"], alpha=0.6)
   plt.plot(front["kl_loss"], front["recon_loss"], linewidth=2)
   plt.scatter([sweet["kl_loss"]], [sweet["recon_loss"]], marker="x", s=120)
   plt.xlabel("KL loss (lower better)")
   plt.ylabel("Reconstruction loss (lower better)")
   plt.title("KL vs Reconstruction — Pareto Front + Sweet Spot")
   plt.tight_layout()
   plt.savefig(outdir / "pareto_plot.png", dpi=180)
   plt.close()
   plt.figure()
   plt.scatter(df["kl_loss"], df["recon_loss"], alpha=0.35)
   plt.plot(front["kl_loss"], front["recon_loss"], linewidth=2)
   plt.scatter([sweet["kl_loss"]], [sweet["recon_loss"]], marker="x", s=120)
   for _, r in front.iterrows():
       plt.annotate(str(r["run_id"]), (r["kl_loss"], r["recon_loss"]), fontsize=7, alpha=0.9)
   plt.xlabel("KL loss")
   plt.ylabel("Reconstruction loss")
   plt.title("Pareto Front (labels = run_id)")
   plt.tight_layout()
...
def main():
   ...
   for fp in files:
       ...
       rows.append(d)
   if not rows:
       raise RuntimeError("No usable metrics_summary.json files found (missing recon_loss/kl_loss).")
   ...
   sweet_out = {"sweet_spot": sweet.to_dict(), "method": "knee", "knee_distance": float(kdist)}
   (outdir / "sweet_spot.json").write_text(json.dumps(sweet_out, indent=2), encoding="utf-8")
